...
enum struct AlpacaErrorCode_t : int32_t
{
    ...
    DriverCommandError = (int32_t)0x00000501,            // The command failed and returned 'false'.
...
private:
    ...
    String _mng_manufacture = "TecnoSky";
    String _mng_manufacture_version = "V1.0";
    String _mng_location = "Italy";
...
#define MYTHROW_RspStatusDriverError(req, rsp_status, command)                                                                              \
    {                                                                                                                                       \
        rsp_status.error_code = AlpacaErrorCode_t::DriverCommandError;                                                                      \
        rsp_status.http_status = HttpStatus_t::kPassed;                                                                                     \
        snprintf(rsp_status.error_msg, sizeof(rsp_status.error_msg), "%s - Command '%s' returned an error", req->url().c_str(), command);   \
        goto mycatch;                                                                                                                       \
    }
};
